...
RegisterServerEvent('esx_dowod:pokadowodzik')
AddEventHandler('esx_dowod:pokadowodzik', function()
    ...
    TriggerClientEvent('esx:dowod_pokazDokument', -1, _source,  '~h~'..name.firstname..' '..name.lastname,'~y~'.. name.dateofbirth .. ' '.. sex .. ' ' .. name.height .. ' cm ','~w~Pozwolenie na broń ' ..jestw.. --[['Ubezpieczenie ' .. ubez ..]] 'Prawo Jazdy ' .. pj)
    TriggerClientEvent('esx_dowod:sendProximityMessage', -1, _source, _source, message)
end)


RegisterServerEvent('esx_dowod:pokawizytowke')
AddEventHandler('esx_dowod:pokawizytowke', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local name = getIdentity(source)
	...
	TriggerClientEvent('esx:dowod_pokazDokument', -1,_source,  'Wizytówka',name.firstname..' '..name.lastname,'~y~Numer Telefonu : ~w~'..name.phone_number)
	TriggerClientEvent('esx_dowod:sendProximityMessage', -1, _source, _source, message)
end)



RegisterServerEvent('esx_dowod:pokaodznake')
AddEventHandler('esx_dowod:pokaodznake', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local job = xPlayer.job
	local name = getIdentity(source)
...
RegisterServerEvent('esx_dowod:pokalegitke')
AddEventHandler('esx_dowod:pokalegitke', function()
local _source = source
local xPlayer = ESX.GetPlayerFromId(_source)
local job = xPlayer.job
local name = getIdentity(source)
...
if job.name == "ambulance" then
	...
	TriggerClientEvent('esx:dowod_pokazDokument', -1,_source, '~h~'..name.firstname..' '..name.lastname,'Legitymacja EMS','Stopień: ~b~'..job.grade_label..'~n~~s~Legitymacja jest ważna: '..czy_wazna)
	TriggerClientEvent('esx_dowod:sendProximityMessage', -1, _source, _source, message)
end)
